    ...
    protected void openCamera(int id) {
        ...
        try {
            ...
            if (mCameraID != null) {
                ...
                double horiz_field_of_view = 2 * Math.atan(.5 * width_dim / focal_lengths[0]);
                double vert_field_of_view = 2 * Math.atan(.5 * height_dim / focal_lengths[0]);
                mView.setFocalLengthPixels(focal_length_pixels);
                mView.setHorizFieldOfViewRad(horiz_field_of_view);
                mView.setVertFieldOfViewRad(vert_field_of_view);
                Log.d(LOGTAG, "Camera focal length (pixels): " + focal_length_pixels);
                Log.d(LOGTAG, "Camera horizontal FOV (deg) " + Math.toDegrees(horiz_field_of_view));
                Log.d(LOGTAG, "Camera vertical   FOV (deg) " + Math.toDegrees(vert_field_of_view));
